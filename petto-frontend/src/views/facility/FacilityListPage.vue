...
export default {
  ...
  methods: {
    filteredList() {
      if (this.selectedAreas == "" && this.selectedAnimals == "")
        return this.$store.state.facilityList;
      ...
      for (var i = 0; i < this.$store.state.facilityList.length; i++) {
        if (this.selectedAreas.length > 0 && this.selectedAnimals.length > 0) {
          ...
          const length =
            selectedAreasLen > selectedAnimalsLen
              ? selectedAreasLen
              : selectedAnimalsLen;

          for (var j = 0; j < length; j++) {
            if (
              this.$store.state.facilityList[i].facilityAddr.includes(
                this.selectedAreas[j]
              ) &&
              this.$store.state.facilityList[i].saveTrgtAnimal.includes(
                this.selectedAnimals[j]
              )
            ) {
